...
procedure TFadcExemplo.dbqtdExit(Sender: TObject);
begin
  dbtotal.Value := dbqtd.Value * dbunit.Value;
end;

procedure TFadcExemplo.dbunitExit(Sender: TObject);
begin
  dbtotal.Value := dbunit.Value * dbqtd.Value;
end;

procedure TFadcExemplo.EdprodutoEnter(Sender: TObject);
begin
  ALT := Edproduto.Text;
end;

procedure TFadcExemplo.EdprodutoExit(Sender: TObject);
VAR
  SQL :STRING;
  VLR :Extended;
  fDVLAOR :tFDQUERY;
...
procedure TFadcExemplo.FormKeyPress(Sender: TObject; var Key: Char);
begin
    if key = #13 then
  begin
    key := #0;
